...
study_config.filenames = {'block001', 'block002', 'block003', 'block004', 'block005'};
if subject_inds == 5
    warning('Add exception for Googles OFF blocks (6 & 7) in order to analyse those');
end
...
study_config.channels_to_remove = {66:67,65:66};
...
study_config.bip_channels  = {'PhotoDiode'};
...
study_config.channel_locations_filename = sprintf('%s_wavegard_withPD.elc', study_config.capName);
...
%% Epoching
study_config.epochs.event = 'TrialStart';
...
study_config.psd.normStyle = 'acrossTrials'; % On which dimensions to apply normalization
...
study_config.class.model = 'svm'; % 'linear', 'diaglinear', 'svm' or 'logistic'
study_config.class.condAnalysis = ''; % only 'separate' for now: analyze GogglesON and GogglesOFF separately
study_config.class.contrast = 'TrialTypePairwise'; % 'TrialType': build a classifier to distinguish between all X trial types
% 'AllvsControl', 'DiscvsControl'
% 'TrialTypePairwise'
...
study_config.class.feat2test = [1,25,50,75,100,125,150,175,200]; % vector of best features to test
%study_config.class.feat2test = [1,100,200,300,400,500,600,700,800,900]; % vector of best features to test
%study_config.class.feat2test = [575:5:625]; % vector of best features to test
...
% Set to 'loo_tr' if you want a Leave-one-out cross validation at the trial
% scale (when chunking)
...
clear included s sourceFile subject_ind tab
