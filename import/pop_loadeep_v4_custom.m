...
function [EEG, command]=pop_loadeep_v4_custom(filename, varargin);
...
N_impedanceMeas = sum(strcmp({r.v4_data.triggers.description},'Impedance'));% Added by AD
EEG.etc.impedanceMeasures = struct('latency', cell(N_impedanceMeas,1),...
    'values',cell(N_impedanceMeas,1));% Added by AD
imp_count = 0;% Added by AD
for i=1:size(r.v4_data.triggers, 2)
  ...
  % Added by AD to extract the impedance measurements automatically
  if ischar(r.v4_data.triggers(i).impedances)
      imp_count = imp_count+1;
      EEG.etc.impedanceMeasures(imp_count).latency = EEG.event(i).latency;
      vals_STR = split(r.v4_data.triggers(i).impedances,' ');
      vals = zeros(length(vals_STR),1);
      for c = 1:length(vals)
          vals(c) = str2double(vals_STR{c});
      end
      EEG.etc.impedanceMeasures(imp_count).values = vals;
  end
end;
