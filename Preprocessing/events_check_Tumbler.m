function EEG = events_check_Tumbler(EEG, cfg)
...
startTrialEvents = strcmp({evts_noBounds.type},'NewTrialSoundOn');
...
for tr = 1:n_Trials
    ...
    if strcmp(subject, 'P1002-2')
        % This subject starts with block 2 (no block 1)
        bl = bl+1;
    end
    tr_inBl = mod(tr-1,blockLength)+1;
...
for tr = 1:n_Trials
    ...
    elseif limits(1) >= limits(2)
        ...
        if (strcmp(subject, 'P1001-3') && any(tr == [19,24])) ||...
                (strcmp(subject, 'P1002-2') && BlockInd(tr) == 8 && TrialInd(tr) == 1) ||...
                (strcmp(subject, 'P1002-3') && BlockInd(tr) == 4 && TrialInd(tr) == 6) ||...
                (strcmp(subject, 'P1001-4') && BlockInd(tr) == 1 && TrialInd(tr) == 6) ||...
                (strcmp(subject, 'P1001-4') && BlockInd(tr) == 1 && TrialInd(tr) == 8) ||...
                (strcmp(subject, 'P1001-4') && BlockInd(tr) == 7 && TrialInd(tr) == 2)
            % These trials should not be taken into account (issue during the recording)
            Trial_perc(tr) = 0;
        end
...
for b = 1:n_Blocks
    ...
    hold on
    if strcmp(subject, 'P1002-2')
        trials_NO_bl = BlockInd == b+1 & contains(TrialType,'Without');
        trials_WO_bl = BlockInd == b+1 & ~contains(TrialType,'Without');
        title(sprintf('Block %d', b+1))
    else
